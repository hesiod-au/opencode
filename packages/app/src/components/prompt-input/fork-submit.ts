 ...
 */
export function createForkPromptOverrides(workspaceDir: string) {
  ...
  return async (input: PromptOverridesInput): Promise<PromptOverrides | undefined> => {
    ...
    if (excludedContent && Object.keys(excludedContent.messages).length > 0) {
      ...
      for (const [msgId, parts] of Object.entries(excludedContent.parts)) {
        ...
        const missing = parts.filter((p) => !existingIds.has(p.id))
        if (missing.length > 0) {
          mergedParts[msgId] = [...existing, ...missing].sort((a, b) => (a.id > b.id ? 1 : -1))
        }
      }
